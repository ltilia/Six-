...
public class SlidingPaneLayout extends ViewGroup {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        for (i = 0; i < childCount; i++) {
            ...
            if (child.getVisibility() == 8) {
                ...
                i2 = lp.width;
                if (r0 == -2) {
                    childWidthSpec = MeasureSpec.makeMeasureSpec(widthAvailable - horizontalMargin, Cue.TYPE_UNSET);
                } else {
                    i2 = lp.width;
                    if (r0 == -1) {
                ...
                i2 = lp.height;
                if (r0 == -2) {
                    childHeightSpec = MeasureSpec.makeMeasureSpec(maxLayoutHeight, Cue.TYPE_UNSET);
                } else {
                    i2 = lp.height;
                    if (r0 == -1) {
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        float x;
        float y;
        switch (ev.getAction() & RadialCountdown.PROGRESS_ALPHA) {
            case Yylex.YYINITIAL /*0*/:
                x = ev.getX();
                y = ev.getY();
                this.mInitialMotionX = x;
    ...
    private void dimChildView(View v, float mag, int fadeColor) {
        ...
        if (mag > 0.0f && fadeColor != 0) {
            int color = (((int) (((float) ((CtaButton.BACKGROUND_COLOR & fadeColor) >>> 24)) * mag)) << 24) | (ViewCompat.MEASURED_SIZE_MASK & fadeColor);
            if (lp.dimPaint == null) {
