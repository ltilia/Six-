...
public class PagerTitleStrip extends ViewGroup implements Decor {
    ...
    public void setNonPrimaryAlpha(@FloatRange(from = 0.0d, to = 1.0d) float alpha) {
        this.mNonPrimaryAlpha = ((int) (255.0f * alpha)) & RadialCountdown.PROGRESS_ALPHA;
    ...
    void updateText(int currentItem, PagerAdapter adapter) {
        ...
        int childWidthSpec = MeasureSpec.makeMeasureSpec(Math.max(0, (int) (((float) ((getWidth() - getPaddingLeft()) - getPaddingRight())) * DefaultLoadControl.DEFAULT_HIGH_BUFFER_LOAD)), Cue.TYPE_UNSET);
        int childHeightSpec = MeasureSpec.makeMeasureSpec(Math.max(0, (getHeight() - getPaddingTop()) - getPaddingBottom()), Cue.TYPE_UNSET);
