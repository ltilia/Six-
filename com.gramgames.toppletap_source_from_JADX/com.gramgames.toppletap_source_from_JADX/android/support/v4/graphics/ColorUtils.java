...
public final class ColorUtils {
    ...
    private static int compositeAlpha(int foregroundAlpha, int backgroundAlpha) {
        return 255 - (((255 - backgroundAlpha) * (255 - foregroundAlpha)) / RadialCountdown.PROGRESS_ALPHA);
    }
    ...
    private static int compositeComponent(int fgC, int fgA, int bgC, int bgA, int a) {
        ...
        return (((fgC * RadialCountdown.PROGRESS_ALPHA) * fgA) + ((bgC * bgA) * (255 - fgA))) / (a * RadialCountdown.PROGRESS_ALPHA);
    }
    ...
    public static double calculateContrast(@ColorInt int foreground, @ColorInt int background) {
        if (Color.alpha(background) != RadialCountdown.PROGRESS_ALPHA) {
            throw new IllegalArgumentException("background can not be translucent: #" + Integer.toHexString(background));
        }
        if (Color.alpha(foreground) < RadialCountdown.PROGRESS_ALPHA) {
    ...
    public static int calculateMinimumAlpha(@ColorInt int foreground, @ColorInt int background, float minContrastRatio) {
        if (Color.alpha(background) != RadialCountdown.PROGRESS_ALPHA) {
            throw new IllegalArgumentException("background can not be translucent: #" + Integer.toHexString(background));
        } else if (calculateContrast(setAlphaComponent(foreground, RadialCountdown.PROGRESS_ALPHA), background) < ((double) minContrastRatio)) {
            return -1;
        } else {
            int minAlpha = 0;
            int maxAlpha = RadialCountdown.PROGRESS_ALPHA;
    ...
    public static int HSLToColor(@NonNull float[] hsl) {
        ...
        return Color.rgb(constrain(r, 0, (int) RadialCountdown.PROGRESS_ALPHA), constrain(g, 0, (int) RadialCountdown.PROGRESS_ALPHA), constrain(b, 0, (int) RadialCountdown.PROGRESS_ALPHA));
    }

    @ColorInt
    public static int setAlphaComponent(@ColorInt int color, @IntRange(from = 0, to = 255) int alpha) {
        if (alpha >= 0 && alpha <= RadialCountdown.PROGRESS_ALPHA) {
    ...
    public static int XYZToColor(@FloatRange(from = 0.0d, to = 95.047d) double x, @FloatRange(from = 0.0d, to = 100.0d) double y, @FloatRange(from = 0.0d, to = 108.883d) double z) {
        ...
        return Color.rgb(constrain((int) Math.round(255.0d * (r > 0.0031308d ? (1.055d * Math.pow(r, 0.4166666666666667d)) - 0.055d : r * 12.92d)), 0, (int) RadialCountdown.PROGRESS_ALPHA), constrain((int) Math.round(255.0d * (g > 0.0031308d ? (1.055d * Math.pow(g, 0.4166666666666667d)) - 0.055d : g * 12.92d)), 0, (int) RadialCountdown.PROGRESS_ALPHA), constrain((int) Math.round(255.0d * (b > 0.0031308d ? (1.055d * Math.pow(b, 0.4166666666666667d)) - 0.055d : b * 12.92d)), 0, (int) RadialCountdown.PROGRESS_ALPHA));
    }
