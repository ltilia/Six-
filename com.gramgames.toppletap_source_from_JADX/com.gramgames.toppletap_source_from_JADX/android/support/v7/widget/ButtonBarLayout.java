...
public class ButtonBarLayout extends LinearLayout {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (isStacked() || MeasureSpec.getMode(widthMeasureSpec) != 1073741824) {
            ...
            initialWidthMeasureSpec = MeasureSpec.makeMeasureSpec(widthSize, Cue.TYPE_UNSET);
            needsRemeasure = true;
        }
        super.onMeasure(initialWidthMeasureSpec, heightMeasureSpec);
        if (this.mAllowStacking && !isStacked() && (getMeasuredWidthAndState() & CtaButton.BACKGROUND_COLOR) == ViewCompat.MEASURED_STATE_TOO_SMALL) {
            setStacked(true);
            needsRemeasure = true;
        }
        if (needsRemeasure) {
