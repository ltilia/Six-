...
public final class WebImage implements SafeParcelable {
    ...
    public WebImage(JSONObject json) throws IllegalArgumentException {
        this(zzj(json), json.optInt(VastIconXmlManager.WIDTH, 0), json.optInt(VastIconXmlManager.HEIGHT, 0));
    }

    private static Uri zzj(JSONObject jSONObject) {
        Uri uri = null;
        if (jSONObject.has(UnityAdsConstants.UNITY_ADS_FAILED_URL_URL_KEY)) {
            try {
                uri = Uri.parse(jSONObject.getString(UnityAdsConstants.UNITY_ADS_FAILED_URL_URL_KEY));
    ...
    public JSONObject toJson() {
        ...
        try {
            jSONObject.put(UnityAdsConstants.UNITY_ADS_FAILED_URL_URL_KEY, this.zzajZ.toString());
            jSONObject.put(VastIconXmlManager.WIDTH, this.zzoG);
            jSONObject.put(VastIconXmlManager.HEIGHT, this.zzoH);
        } catch (JSONException e) {
        }
        return jSONObject;
    }
