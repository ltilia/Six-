...
public class zzt {
    ...
    public zzt(Context context, VersionInfoParcel versionInfoParcel, String str, @Nullable zzcb com_google_android_gms_internal_zzcb, @Nullable zzbz com_google_android_gms_internal_zzbz) {
        this.zzFc = new zzb().zza("min_1", Double.MIN_VALUE, 1.0d).zza("1_5", 1.0d, 5.0d).zza("5_10", 5.0d, 10.0d).zza("10_20", 10.0d, 20.0d).zza("20_30", 20.0d, 30.0d).zza("30_max", 30.0d, Double.MAX_VALUE).zzhA();
        this.zzFn = -1;
    ...
    public void onStop() {
        if (((Boolean) zzbt.zzvU.get()).booleanValue() && !this.zzFl) {
            ...
            bundle.putString(UnityAdsConstants.UNITY_ADS_ANALYTICS_QUERYPARAM_EVENTTYPE_KEY, "native-player-metrics");
    ...
    String zza(TextureView textureView) {
        ...
        while (i < 8) {
            ...
            for (int i2 = 0; i2 < 8; i2++) {
                ...
                j3 |= (Color.green(pixel) + (Color.blue(pixel) + Color.red(pixel)) > RadialCountdown.BACKGROUND_ALPHA ? 1 : 0) << ((int) j);
                j--;
            }
            i++;
            j2 = j;
