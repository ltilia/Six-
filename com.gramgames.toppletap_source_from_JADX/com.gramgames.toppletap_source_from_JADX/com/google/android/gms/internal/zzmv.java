...
public final class zzmv {
    ...
    static {
        zzaob = new IntentFilter("android.intent.action.BATTERY_CHANGED");
        zzaod = Float.NaN;
    }
    ...
    public static synchronized float zzay(Context context) {
        ...
        synchronized (zzmv.class) {
            if (SystemClock.elapsedRealtime() - zzaoc >= HlsChunkSource.DEFAULT_PLAYLIST_BLACKLIST_MS || zzaod == Float.NaN) {
                Intent registerReceiver = context.getApplicationContext().registerReceiver(null, zzaob);
                if (registerReceiver != null) {
                    zzaod = ((float) registerReceiver.getIntExtra(AppLovinEventTypes.USER_COMPLETED_LEVEL, -1)) / ((float) registerReceiver.getIntExtra("scale", -1));
                }
