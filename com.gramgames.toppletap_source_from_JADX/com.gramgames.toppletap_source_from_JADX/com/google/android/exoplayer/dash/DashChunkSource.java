...
public class DashChunkSource implements ChunkSource, Output {
            ...
            if (queue.isEmpty()) {
                if (this.live) {
                    if (this.startAtLiveEdge) {
                        ...
                        long min = Math.min(playbackPositionUs, this.availableRangeValues[1] - 1);
                        playbackPositionUs = Math.max(playbackPositionUs, this.availableRangeValues[0]);
                    }
                }
    ...
    private TimeRange getAvailableRange(long nowUnixTimeUs) {
        ...
        return new DynamicTimeRange(firstPeriod.getAvailableStartTimeUs(), lastPeriod.isIndexUnbounded() ? PtsTimestampAdjuster.DO_NOT_OFFSET : lastPeriod.getAvailableEndTimeUs(), (this.systemClock.elapsedRealtime() * 1000) - (nowUnixTimeUs - (this.currentManifest.availabilityStartTime * 1000)), this.currentManifest.timeShiftBufferDepth == -1 ? -1 : this.currentManifest.timeShiftBufferDepth * 1000, this.systemClock);
    }

    private void notifyAvailableRangeChanged(TimeRange seekRange) {
        if (this.eventHandler != null && this.eventListener != null) {
            this.eventHandler.post(new 1(seekRange));
        }
    }
}
