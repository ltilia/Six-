...
public class VastManager implements VastXmlManagerAggregatorListener {
    ...
    class 1 implements VideoDownloaderListener {
        final /* synthetic */ VastVideoConfig val$vastVideoConfig;

        1(VastVideoConfig vastVideoConfig) {
            this.val$vastVideoConfig = vastVideoConfig;
        }

        public void onComplete(boolean success) {
            if (success && VastManager.this.updateDiskMediaFileUrl(this.val$vastVideoConfig)) {
                VastManager.this.mVastManagerListener.onVastVideoConfigurationPrepared(this.val$vastVideoConfig);
                return;
            }
    ...
    public void onAggregationComplete(@Nullable VastVideoConfig vastVideoConfig) {
        if (this.mVastManagerListener == null) {
            ...
            VideoDownloader.cache(vastVideoConfig.getNetworkMediaFileUrl(), new 1(vastVideoConfig));
        }
    }
