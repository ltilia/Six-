...
public class VideoViewabilityTrackerXmlManager {
    ...
    Integer getViewablePlaytimeMS() {
        ...
        if (Strings.isAbsoluteTracker(viewablePlaytimeStr)) {
            try {
                ...
                MoPubLog.d(String.format("Invalid VAST viewablePlaytime format for \"HH:MM:SS[.mmm]\": %s:", new Object[]{viewablePlaytimeStr}));
            }
        } else {
            try {
                viewablePlaytimeMS = Integer.valueOf((int) (Float.parseFloat(viewablePlaytimeStr) * 1000.0f));
            } catch (NumberFormatException e2) {
                MoPubLog.d(String.format("Invalid VAST viewablePlaytime format for \"SS[.mmm]\": %s:", new Object[]{viewablePlaytimeStr}));
            }
        }
    ...
    Integer getPercentViewable() {
        ...
        try {
            percentViewable = Integer.valueOf((int) Float.parseFloat(percentViewableStr.replace("%", BuildConfig.FLAVOR)));
        } catch (NumberFormatException e) {
            MoPubLog.d(String.format("Invalid VAST percentViewable format for \"d{1,3}%%\": %s:", new Object[]{percentViewableStr}));
        }
