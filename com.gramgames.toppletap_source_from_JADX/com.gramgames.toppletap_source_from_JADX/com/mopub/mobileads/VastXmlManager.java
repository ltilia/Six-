...
class VastXmlManager {
    ...
    void parseVastXml(@NonNull String xmlString) throws ParserConfigurationException, IOException, SAXException {
        ...
        String documentString = ROOT_TAG_OPEN + xmlString.replaceFirst("<\\?.*\\?>", BuildConfig.FLAVOR) + ROOT_TAG_CLOSE;
    ...
    String getCustomCtaText() {
        ...
        return (customCtaText == null || customCtaText.length() > MAX_CTA_TEXT_LENGTH) ? null : customCtaText;
    }

    @Nullable
    String getCustomSkipText() {
        String customSkipText = XmlUtils.getFirstMatchingStringData(this.mVastDoc, CUSTOM_SKIP_TEXT);
        return (customSkipText == null || customSkipText.length() > MAX_SKIP_TEXT_LENGTH) ? null : customSkipText;
    }
