...
public class VastXmlManagerAggregator extends AsyncTask<String, Void, VastVideoConfig> {
    ...
    static {
        VIDEO_MIME_TYPES = Arrays.asList(new String[]{MimeTypes.VIDEO_MP4, MimeTypes.VIDEO_H263});
    }
    ...
    VastCompanionAdConfig getBestCompanionAd(@NonNull List<VastCompanionAdXmlManager> managers, @NonNull CompanionOrientation orientation) {
        ...
        for (Type type : Type.values()) {
            for (VastCompanionAdXmlManager companionXmlManager : arrayList) {
                ...
                if (width != null && width.intValue() >= MINIMUM_COMPANION_AD_WIDTH && height != null && height.intValue() >= MINIMUM_COMPANION_AD_HEIGHT) {
                ...
                if (ADS_BY_AD_SLOT_ID.equals(adSlotId)) {
                    if (width.intValue() >= 25) {
                        if (width.intValue() <= 75) {
                            if (height.intValue() >= MAX_TIMES_TO_FOLLOW_VAST_REDIRECT) {
                                if (height.intValue() > 50) {
                                }
                            }
                        }
                    }
                } else if (SOCIAL_ACTIONS_AD_SLOT_ID.equals(adSlotId) && width.intValue() >= 50 && width.intValue() <= CtaButton.WIDTH_DIPS && height.intValue() >= MAX_TIMES_TO_FOLLOW_VAST_REDIRECT) {
                    if (height.intValue() > 50) {
                    }
                }
    ...
    VastIconConfig getBestIcon(@NonNull List<VastIconXmlManager> managers) {
        ...
        for (Type type : Type.values()) {
            for (VastIconXmlManager iconXmlManager : iconXmlManagers) {
                ...
                if (width != null && width.intValue() > 0 && width.intValue() <= MINIMUM_COMPANION_AD_WIDTH && height != null && height.intValue() > 0 && height.intValue() <= MINIMUM_COMPANION_AD_WIDTH) {
    ...
    private String followVastRedirect(@NonNull String redirectUrl) throws IOException {
        ...
        if (this.mTimesFollowedVastRedirect >= MAX_TIMES_TO_FOLLOW_VAST_REDIRECT) {
            return null;
        }
