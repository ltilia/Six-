            ...
            for (Node progressNode : XmlUtils.getMatchingChildNodes(trackingEvents, VIDEO_TRACKER, EVENT, Collections.singletonList(PROGRESS))) {
                ...
                if (offsetString != null) {
                    ...
                    if (Strings.isPercentageTracker(offsetString)) {
                        try {
                            percentTrackers.add(new VastFractionalProgressTracker(XmlUtils.getNodeValue(progressNode), Float.parseFloat(offsetString.replace("%", BuildConfig.FLAVOR)) / 100.0f));
                        } catch (NumberFormatException e) {
                            MoPubLog.d(String.format("Failed to parse VAST progress tracker %s", new Object[]{offsetString}));
                        }
                    }
                }
            }
    ...
    List<VastAbsoluteProgressTracker> getAbsoluteProgressTrackers() {
        ...
        if (trackingEvents != null) {
            ...
            for (Node creativeViewNode : XmlUtils.getMatchingChildNodes(trackingEvents, VIDEO_TRACKER, EVENT, Collections.singletonList(CREATIVE_VIEW))) {
                trackers.add(new VastAbsoluteProgressTracker(XmlUtils.getNodeValue(creativeViewNode), CREATIVE_VIEW_TRACKER_THRESHOLD));
            }
        }
