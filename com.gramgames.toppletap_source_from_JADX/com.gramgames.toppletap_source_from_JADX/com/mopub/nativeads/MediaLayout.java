...
public class MediaLayout extends RelativeLayout {
    ...
    public void initForVideo() {
        if (!this.mIsInitialized) {
            ...
            loadingSpinnerParams.addRule(PINNER_PADDING_DIPS);
            ...
            this.mVideoProgress.calibrateAndMakeVisible(AdError.NETWORK_ERROR_CODE, 0);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        } else if (widthMode == Cue.TYPE_UNSET) {
        ...
        if (Math.abs(finalHeight - curHeight) >= 2 || Math.abs(finalWidth - curWidth) >= 2) {
            MoPubLog.v(String.format("Resetting mediaLayout size to w: %d h: %d", new Object[]{Integer.valueOf(finalWidth), Integer.valueOf(finalHeight)}));
    ...
    public void setMuteState(@NonNull MuteState muteState) {
        ...
        if (muteState != this.mMuteState) {
            ...
            if (this.mMuteControl != null) {
                switch (2.$SwitchMap$com$mopub$nativeads$MediaLayout$MuteState[this.mMuteState.ordinal()]) {
    ...
    private void updateViewState() {
        switch (2.$SwitchMap$com$mopub$nativeads$MediaLayout$Mode[this.mMode.ordinal()]) {
            case Yytoken.TYPE_LEFT_BRACE /*1*/:
