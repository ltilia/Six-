...
public class MoPubStreamAdPlacer {
    ...
    static class 1 implements MoPubNativeAdLoadedListener {
        1() {
        }
    ...
    class 3 implements PositioningListener {
        3() {
        }
    ...
    class 4 implements AdSourceListener {
        4() {
        }
    ...
    static {
        EMPTY_NATIVE_AD_LOADED_LISTENER = new 1();
    }
    ...
    MoPubStreamAdPlacer(@NonNull Activity activity, @NonNull NativeAdSource adSource, @NonNull PositioningSource positioningSource) {
        ...
        this.mPlacementRunnable = new 2();
        this.mVisibleRangeStart = CONTENT_VIEW_TYPE;
        this.mVisibleRangeEnd = CONTENT_VIEW_TYPE;
    }
    ...
    public void loadAds(@NonNull String adUnitId, @Nullable RequestParameters requestParameters) {
        ...
        this.mPositioningSource.loadPositions(adUnitId, new 3());
        this.mAdSource.setAdSourceListener(new 4());
    ...
    private void placeInitialAds(PlacementData placementData) {
        removeAdsInRange(CONTENT_VIEW_TYPE, this.mItemCount);
    ...
    public void placeAdsInRange(int startPosition, int endPosition) {
        ...
        this.mVisibleRangeEnd = Math.min(endPosition, startPosition + MAX_VISIBLE_RANGE);
        notifyNeedsPlacement();
    }
    ...
    public void destroy() {
        this.mPlacementHandler.removeMessages(CONTENT_VIEW_TYPE);
        this.mAdSource.clear();
    ...
    public int removeAdsInRange(int originalStartPosition, int originalEndPosition) {
        ...
        for (int i = positions.length + DEFAULT_AD_VIEW_TYPE; i >= 0; i += DEFAULT_AD_VIEW_TYPE) {
            ...
            if (position >= adjustedStartRange && position < adjustedEndRange) {
                ...
                if (position < this.mVisibleRangeStart) {
                    this.mVisibleRangeStart += DEFAULT_AD_VIEW_TYPE;
                }
                this.mItemCount += DEFAULT_AD_VIEW_TYPE;
            }
        }
    ...
    private void placeAds() {
        if (tryPlaceAdsInRange(this.mVisibleRangeStart, this.mVisibleRangeEnd)) {
            tryPlaceAdsInRange(this.mVisibleRangeEnd, this.mVisibleRangeEnd + RANGE_BUFFER);
        }
    }

    private boolean tryPlaceAdsInRange(int start, int end) {
        int position = start;
        int lastPosition = end + DEFAULT_AD_VIEW_TYPE;
        while (position <= lastPosition && position != DEFAULT_AD_VIEW_TYPE && position < this.mItemCount) {
