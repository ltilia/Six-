...
public class ScribeEventRecorder implements EventRecorder {
    ...
    class 1 implements ScribeRequestFactory {
        ...
        public ScribeRequest createRequest(Listener listener) {
            return new ScribeRequest(ScribeEventRecorder.SCRIBE_URL, this.val$events, ScribeEventRecorder.this.mEventSerializer, listener);
        }
    }
    ...
    void sendEvents() {
        if (!this.mScribeRequestManager.isAtCapacity()) {
            List<BaseEvent> events = dequeueEvents();
            if (!events.isEmpty()) {
                this.mScribeRequestManager.makeRequest(new 1(events), new ScribeBackoffPolicy());
            }
        }
    }
    ...
    List<BaseEvent> dequeueEvents() {
        ...
        while (this.mEventQueue.peek() != null && baseEvents.size() < EVENT_COUNT_SEND_THRESHHOLD) {
