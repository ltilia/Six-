package com.amazon.device.ads;

import com.amazon.device.ads.FileOutputHandler.WriteMethod;
import java.io.File;
import java.io.FileOutputStream;
import java.io.IOException;
import java.io.OutputStream;

class Assets {
    public static final String CLOSE = "amazon_ads_close.png";
    public static final String CLOSE_NORMAL = "amazon_ads_close_normal.png";
    public static final String CLOSE_PRESSED = "amazon_ads_close_pressed.png";
    public static final String LEFT_ARROW = "amazon_ads_leftarrow.png";
    private static final String LOGTAG;
    public static final String OPEN_EXTERNAL_BROWSER = "amazon_ads_open_external_browser.png";
    public static final String REFRESH = "amazon_ads_refresh.png";
    public static final String RIGHT_ARROW = "amazon_ads_rightarrow.png";
    private static final int VERSION = 5;
    private static final String VERSION_SETTINGS_KEY = "mraid-assets-version";
    private static Assets instance;
    private String baseFilePath;
    private final FileHandlerFactory fileHandlerFactory;
    private final MobileAdsInfoStore infoStore;
    private final MobileAdsLogger logger;
    private final Metrics metrics;
    private final Settings settings;

    static class AssetsBase64 {
        public static final String BASE64_CLOSE = "";
        public static final String BASE64_CLOSE_NORMAL = "";
        public static final String BASE64_CLOSE_PRESSED = "";
        public static final String BASE64_INTERSTITIAL_CLOSE_NORMAL = "";
        public static final String BASE64_INTERSTITIAL_CLOSE_PRESSED = "";
        public static final String BASE64_LEFT_ARROW = "";
        public static final String BASE64_OPEN_EXTERNAL_BROWSER = "";
        public static final String BASE64_REFRESH = "";
        public static final String BASE64_RIGHT_ARROW = "";

        AssetsBase64() {
        }
    }

    class CircuitBreaker {
        boolean broken;

        CircuitBreaker() {
            this.broken = false;
        }

        private void breakCircuit() {
            this.broken = true;
        }

        private boolean isBroken() {
            return this.broken;
        }
    }

    static {
        LOGTAG = Assets.class.getSimpleName();
        instance = new Assets(MobileAdsInfoStore.getInstance(), new DefaultFileHandlerFactory(), Settings.getInstance(), Metrics.getInstance());
    }

    protected Assets(MobileAdsInfoStore infoStore, FileHandlerFactory fileHandlerFactory, Settings settings, Metrics metrics) {
        this.logger = new MobileAdsLoggerFactory().createMobileAdsLogger(LOGTAG);
        this.infoStore = infoStore;
        this.fileHandlerFactory = fileHandlerFactory;
        this.settings = settings;
        this.metrics = metrics;
        this.baseFilePath = getBaseFilePath();
    }

    public static final Assets getInstance() {
        return instance;
    }

    public String getFilePath(String file) {
        return this.baseFilePath + file;
    }

    protected String getBaseFilePath() {
        if (this.infoStore == null || this.infoStore.getFilesDir() == null) {
            return null;
        }
        return this.infoStore.getFilesDir().getAbsolutePath() + File.separator;
    }

    /* JADX WARNING: inconsistent code. */
    /* Code decompiled incorrectly, please refer to instructions dump. */
    public synchronized boolean ensureAssetsCreated() {
        /*
        r11 = this;
        r5 = 1;
        r9 = 5;
        r4 = 0;
        monitor-enter(r11);
        r6 = r11.baseFilePath;	 Catch:{ all -> 0x0069 }
        if (r6 != 0) goto L_0x0014;
    L_0x0008:
        r6 = r11.getBaseFilePath();	 Catch:{ all -> 0x0069 }
        r11.baseFilePath = r6;	 Catch:{ all -> 0x0069 }
        r6 = r11.baseFilePath;	 Catch:{ all -> 0x0069 }
        if (r6 != 0) goto L_0x0014;
    L_0x0012:
        monitor-exit(r11);
        return r4;
    L_0x0014:
        r2 = java.lang.System.nanoTime();	 Catch:{ all -> 0x0069 }
        r6 = r11.settings;	 Catch:{ all -> 0x0069 }
        r7 = "mraid-assets-version";
        r8 = 0;
        r1 = r6.getInt(r7, r8);	 Catch:{ all -> 0x0069 }
        r0 = 0;
        if (r1 >= r9) goto L_0x0045;
    L_0x0024:
        r6 = r11.logger;	 Catch:{ all -> 0x0069 }
        r7 = "Writing assets to disk: version %d";
        r8 = 1;
        r8 = new java.lang.Object[r8];	 Catch:{ all -> 0x0069 }
        r9 = 0;
        r10 = 5;
        r10 = java.lang.Integer.valueOf(r10);	 Catch:{ all -> 0x0069 }
        r8[r9] = r10;	 Catch:{ all -> 0x0069 }
        r6.d(r7, r8);	 Catch:{ all -> 0x0069 }
        r6 = r11.writeAssetsToDisk();	 Catch:{ all -> 0x0069 }
        if (r6 == 0) goto L_0x005d;
    L_0x003c:
        r4 = r11.settings;	 Catch:{ all -> 0x0069 }
        r6 = "mraid-assets-version";
        r7 = 5;
        r4.putInt(r6, r7);	 Catch:{ all -> 0x0069 }
        r0 = 1;
    L_0x0045:
        r6 = java.lang.System.nanoTime();	 Catch:{ all -> 0x0069 }
        r6 = r6 - r2;
        r2 = com.amazon.device.ads.NumberUtils.convertToMillisecondsFromNanoseconds(r6);	 Catch:{ all -> 0x0069 }
        r4 = r11.metrics;	 Catch:{ all -> 0x0069 }
        r6 = r4.getMetricsCollector();	 Catch:{ all -> 0x0069 }
        if (r0 == 0) goto L_0x006c;
    L_0x0056:
        r4 = com.amazon.device.ads.Metrics.MetricType.ASSETS_CREATED_LATENCY;	 Catch:{ all -> 0x0069 }
    L_0x0058:
        r6.publishMetricInMilliseconds(r4, r2);	 Catch:{ all -> 0x0069 }
        r4 = r5;
        goto L_0x0012;
    L_0x005d:
        r5 = r11.metrics;	 Catch:{ all -> 0x0069 }
        r5 = r5.getMetricsCollector();	 Catch:{ all -> 0x0069 }
        r6 = com.amazon.device.ads.Metrics.MetricType.ASSETS_FAILED;	 Catch:{ all -> 0x0069 }
        r5.incrementMetric(r6);	 Catch:{ all -> 0x0069 }
        goto L_0x0012;
    L_0x0069:
        r4 = move-exception;
        monitor-exit(r11);
        throw r4;
    L_0x006c:
        r4 = com.amazon.device.ads.Metrics.MetricType.ASSETS_ENSURED_LATENCY;	 Catch:{ all -> 0x0069 }
        goto L_0x0058;
        */
        throw new UnsupportedOperationException("Method not decompiled: com.amazon.device.ads.Assets.ensureAssetsCreated():boolean");
    }

    private boolean writeAssetsToDisk() {
        CircuitBreaker breaker = new CircuitBreaker();
        writeBase64EncodedAssetToDisk(breaker, AssetsBase64.BASE64_CLOSE_NORMAL, getFilePath(CLOSE_NORMAL));
        writeBase64EncodedAssetToDisk(breaker, AssetsBase64.BASE64_CLOSE_PRESSED, getFilePath(CLOSE_PRESSED));
        writeBase64EncodedAssetToDisk(breaker, AssetsBase64.BASE64_CLOSE, getFilePath(CLOSE));
        writeBase64EncodedAssetToDisk(breaker, AssetsBase64.BASE64_LEFT_ARROW, getFilePath(LEFT_ARROW));
        writeBase64EncodedAssetToDisk(breaker, AssetsBase64.BASE64_REFRESH, getFilePath(REFRESH));
        writeBase64EncodedAssetToDisk(breaker, AssetsBase64.BASE64_RIGHT_ARROW, getFilePath(RIGHT_ARROW));
        writeBase64EncodedAssetToDisk(breaker, AssetsBase64.BASE64_INTERSTITIAL_CLOSE_NORMAL, getFilePath(CLOSE_NORMAL));
        writeBase64EncodedAssetToDisk(breaker, AssetsBase64.BASE64_INTERSTITIAL_CLOSE_PRESSED, getFilePath(CLOSE_PRESSED));
        writeBase64EncodedAssetToDisk(breaker, AssetsBase64.BASE64_OPEN_EXTERNAL_BROWSER, getFilePath(OPEN_EXTERNAL_BROWSER));
        return !breaker.isBroken();
    }

    private void writeBase64EncodedAssetToDisk(CircuitBreaker breaker, String base64String, String filename) {
        writeAssetToDisk(breaker, Base64.decode(base64String), filename);
    }

    private void writeAssetToDisk(CircuitBreaker breaker, byte[] bytes, String filename) {
        if (!breaker.isBroken()) {
            FileOutputHandler fileOutputHandler = this.fileHandlerFactory.createFileOutputHandler(filename);
            if (fileOutputHandler == null) {
                this.logger.e("Could not create output handler for asset file.");
                return;
            }
            if (fileOutputHandler.open(WriteMethod.OVERWRITE)) {
                try {
                    fileOutputHandler.write(bytes);
                } catch (IOException e) {
                    this.logger.w("Caught IOException while writing asset to disk: %s", filename);
                    breaker.breakCircuit();
                }
            } else {
                breaker.breakCircuit();
            }
            fileOutputHandler.close();
        }
    }

    protected OutputStream createOutputStream(String filename) throws IOException {
        return new FileOutputStream(filename);
    }

    protected void writeBytesToOutputStream(OutputStream outputStream, byte[] bytes) throws IOException {
        outputStream.write(bytes);
    }
}
