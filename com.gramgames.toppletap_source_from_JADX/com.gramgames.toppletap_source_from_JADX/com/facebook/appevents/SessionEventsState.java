...
class SessionEventsState {
    ...
    public SessionEventsState(AttributionIdentifiers identifiers, String anonymousGUID) {
        ...
        this.MAX_ACCUMULATED_LOG_EVENTS = AdError.NETWORK_ERROR_CODE;
    ...
    public synchronized void addEvent(AppEvent event) {
        if (this.accumulatedEvents.size() + this.inFlightEvents.size() >= AdError.NETWORK_ERROR_CODE) {
    ...
    private byte[] getStringAsByteArray(String jsonString) {
        byte[] jsonUtf8 = null;
        try {
            jsonUtf8 = jsonString.getBytes(C.UTF8_NAME);
        } catch (Exception e) {
