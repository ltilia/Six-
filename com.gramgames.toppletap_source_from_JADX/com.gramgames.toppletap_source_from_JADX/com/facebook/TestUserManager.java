...
public class TestUserManager {
    ...
    private AccessToken getAccessTokenForUser(List<String> permissions, Mode mode, String uniqueUserTag) {
        ...
        return new AccessToken(testAccount.optString(ServerProtocol.DIALOG_PARAM_ACCESS_TOKEN), this.testApplicationId, testAccount.optString(UnityAdsConstants.UNITY_ADS_ZONE_ID_KEY), permissions, null, AccessTokenSource.TEST_USER, null, null);
    }
    ...
    private synchronized void retrieveTestAccountsForAppIfNeeded() {
        if (this.appTestAccounts == null) {
            ...
            testUserNamesParam.putString(GraphRequest.FIELDS_PARAM, UnityAdsConstants.UNITY_ADS_ZONE_NAME_KEY);
            new GraphRequest(null, BuildConfig.FLAVOR, testUserNamesParam, null).setBatchEntryDependsOn("testUsers");
            ...
            populateTestAccounts(((GraphResponse) responses.get(0)).getJSONObject().optJSONArray(UnityAdsConstants.UNITY_ADS_JSON_DATA_ROOTKEY), ((GraphResponse) responses.get(1)).getJSONObject());
        }
    }
    ...
    private synchronized void storeTestAccount(JSONObject testAccount) {
        this.appTestAccounts.put(testAccount.optString(UnityAdsConstants.UNITY_ADS_ZONE_ID_KEY), testAccount);
    }
    ...
    private synchronized JSONObject findTestAccountMatchingIdentifier(String identifier) {
        ...
        for (JSONObject testAccount2 : this.appTestAccounts.values()) {
            if (testAccount2.optString(UnityAdsConstants.UNITY_ADS_ZONE_NAME_KEY).contains(identifier)) {
                break;
            }
        }
        testAccount2 = null;
    ...
    private JSONObject createTestAccount(List<String> permissions, Mode mode, String uniqueUserTag) {
        ...
        parameters.putString(ServerProtocol.DIALOG_PARAM_ACCESS_TOKEN, getAppAccessToken());
        if (mode == Mode.SHARED) {
            parameters.putString(UnityAdsConstants.UNITY_ADS_ZONE_NAME_KEY, String.format("Shared %s Testuser", new Object[]{getSharedTestAccountIdentifier(permissions, uniqueUserTag)}));
        }
        ...
        if (!$assertionsDisabled && testAccount == null) {
            ...
            try {
                testAccount.put(UnityAdsConstants.UNITY_ADS_ZONE_NAME_KEY, parameters.getString(UnityAdsConstants.UNITY_ADS_ZONE_NAME_KEY));
