...
public class AppLinkNavigation {
    ...
    static class 1 implements Continuation<AppLink, NavigationResult> {
    ...
    public NavigationResult navigate(Context context) {
        ...
        for (Target target : getAppLink().getTargets()) {
            ...
            if (pm.resolveActivity(targetIntent, ExecutionOptions.MAX_TRACKING_TAG_STRING_LENGTH) != null) {
    ...
    private void sendAppLinkNavigateEventBroadcast(Context context, Intent intent, NavigationResult type, JSONException e) {
        ...
        extraLoggingData.put(UnityAdsConstants.UNITY_ADS_ANALYTICS_QUERYPARAM_EVENTTYPE_KEY, type.getCode());
    ...
    public static Task<NavigationResult> navigateInBackground(Context context, Uri destination, AppLinkResolver resolver) {
        return resolver.getAppLinkFromUrlInBackground(destination).onSuccess(new 1(context), Task.UI_THREAD_EXECUTOR);
    }
